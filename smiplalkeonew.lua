...
changelogtext = u8[[
Версия 3.9.5 только для сервера Yuma.
Исправлены номера волн в эфирах. 
Исправлен текст собеседования.
Исправлены вопросы для интервью
...
Скрипт был разработан для Arizona RP]])
			...
			imgui.Text(u8'Если у вас есть какие-то проблемы с хелпером напишите заместителю директора')
			if imgui.Button(u8'Сообщить о проблеме') then
				if not isMonetLoader() then
			...
			if imgui.IsItemHovered() then imgui.SetTooltip(u8"Кликните ЛКМ, чтобы написать заместителю директора")  end -- для сообщения о трабле/баге
...
function main()
	...
	while true do wait(0)
		...
		if sampIsLocalPlayerSpawned() and spawn then
			...
			sampAddChatMessage("[ИНФОРМАЦИЯ] {FFFFFF}Последее обновление: "..curcolor.."{008000} 09.07.2024", curcolor1)
...
function update()
	...
	color = curcolor1
	local file, code = downloadFileToPath2('https://raw.githubusercontent.com/nightooes/script/main/cnnhelperupdate.lua', thisScript().path)
	if file then
