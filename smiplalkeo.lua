...
Информация о обновлениях не заполняется.
]]
...
pro_vopr_text = {u8'Как обозначаются автомобили в объявлениях?', u8'Как обозначаются мотоциклы в объявлениях?', u8'Как обозначаются верталеты в объявлениях?', u8'Как отредактируете объявления:', 
...
interv_quest = {u8'Сегодня я хочу взять у вас интервью на тему "Вопрос = ответ". Вы согласны?', u8'Расскажите о себе.', u8'Есть ли у вас жена, дети?',

				u8'Есть ли у вас дом, автомобиль?', u8'Хотели бы вы передать кому-нибудь приветы?', u8'Сколько вы стоите на данной должности?',

				u8'Как ваше настроение?', u8'Расскажите о себе.',
...
function colored_button_main(r, g, b)
	...
	if smi_menu == 'default' then
		...
		if imgui.Button(faicons('key') .. u8' Основное', (imgui.ImVec2(sizeX, sizeY))) then smi_menu = 'default' end
	...
	if smi_menu == 'sobes' then
		...
		if imgui.Button(faicons('passport') .. u8' Собеседование', (imgui.ImVec2(sizeX, sizeY))) then smi_menu = 'sobes' end
	...
	if smi_menu == 'efir' then
		...
		if imgui.Button(faicons('headset') .. u8' Эфиры', (imgui.ImVec2(sizeX, sizeY))) then smi_menu = 'efir' end
	...
	if smi_menu == 'settings' then
		...
		if imgui.Button(faicons('gears') ..u8' Настройки', (imgui.ImVec2(sizeX, sizeY))) then smi_menu = 'settings' end
	...
	if smi_menu == 'dopka' then
		...
		if imgui.Button(faicons('wrench') .. u8' Доп. функции', (imgui.ImVec2(sizeX, sizeY))) then smi_menu = 'dopka' end
	...
	if smi_menu == 'pro' then
		...
		if imgui.Button(faicons('note_sticky') .. u8' ПРО/ППЭ/Устав', (imgui.ImVec2(sizeX, sizeY))) then smi_menu = 'pro' end
		...
		if imgui.Button(faicons('shield_check') .. u8' Руководство ', (imgui.ImVec2(sizeX, sizeY))) then smi_menu = 'ryk' end
	...
	if smi_menu == 'about' then
		...
		if imgui.Button(faicons('file_circle_info') .. u8' О скрипте', (imgui.ImVec2(sizeX, sizeY))) then smi_menu = 'about' end
...
local edit_h = imgui.OnFrame(
	...
	function(self)
		...
		imgui.SetNextWindowSize(imgui.ImVec2(320 * MONET_DPI_SCALE, 320 * MONET_DPI_SCALE)) -- 200
...
local newFrame = imgui.OnFrame(
	...
	function(self)
		...
		local sizeX, sizeY = 600 * MONET_DPI_SCALE, 350 * MONET_DPI_SCALE
		...
		if fast_menu == 'sobes' then
			...
			if imgui.Button(faicons('FILE_ALT') .. u8" Попросить документы", imgui.ImVec2(-1, 25 * MONET_DPI_SCALE)) then
				...
				if fastmenuID > -1 and fastmenuID <= 1000 and sampIsPlayerConnected(fastmenuID) then
					...
					lua_thread.create(function ()
						...
						sampSendChat("/b Отыгровки РП должны быть!")

						wait(2000)

						sampSendChat("/n /me , /do, /todo")
			...
			if imgui.Button(faicons('QUESTION') .. u8" Расскажите о себе", imgui.ImVec2(-1, 25 * MONET_DPI_SCALE)) then
				...
				lua_thread.create(function ()
					...
					sampSendChat("Хорошо, мне нужно узнать больше о вас.")

					wait(2000)

					sampSendChat("Расскажите о себе. Чем занимаетесь? Кем работали ранее?")
			...
			if imgui.Button(faicons('QUESTION') .. u8" Редактирование объявлений?", imgui.ImVec2(-1, 25 * MONET_DPI_SCALE)) then

				sampSendChat("Умеете ли вы редактировать объявления?")
				...
				if fastmenuID > -1 and fastmenuID <= 1000 and sampIsPlayerConnected(fastmenuID) then
					...
					lua_thread.create(function ()
						...
						sobes_info = {
							...
							lic = u8'Не проверено (работает через раз)'
...
local main_menu = imgui.OnFrame(
	...
	function(self)
		...
		imgui.SetNextWindowSize(imgui.ImVec2(800 * MONET_DPI_SCALE, 300 * MONET_DPI_SCALE))
		...
		if smi_menu == 'default' then
			...
			imgui.BeginChild('a', imgui.ImVec2(-1, 233 * MONET_DPI_SCALE), true)
				...
				imgui.Text(u8'В новом обновлении была произведена работа с размерами окон.')

				imgui.Text(u8'Если вы заметили ошибку свяжитесь с Нейтом Бейсоном через меню "Руководство"')
		...
		if smi_menu == 'sobes' then
			...
			if imgui.Button(faicons('QUESTION') .. u8" Расскажите о себе", imgui.ImVec2(-1, 25 * MONET_DPI_SCALE)) then
				...
				lua_thread.create(function ()
					...
					sampSendChat("Кем работали ранее? Чем занимаетесь?.")
			...
			if imgui.Button(u8'Начать эфир', (imgui.ImVec2(200 * MONET_DPI_SCALE, 20 * MONET_DPI_SCALE))) then
				...
				if mainIni.config.c_nick ~= nil then
					...
					lua_thread.create(function()
						...
						wait(6000)
						sampSendChat('/news [Собеседование] У нас есть нововведение об этом я расскажу позже! Дождитесь! ')
						wait(6000)
						...
						wait(6000)
						sampSendChat('/news [Собеседование] Хочу рассказать вам о нововведении.')
						wait(6000)
						sampSendChat('/news [Собеседование] Теперь мы сразу принимаем на 3-ю порядковую должность через собеседование')
						wait(6000)
						...
						wait(6000)
						sampSendChat('/news [Собеседование] Ещё увидимся, штат!')
						wait(6000)
						sampSendChat('/news •°•°•°•° Музыкальная заставка радиостанции « СМИ г. Лос-Сантос » °•°•°')
						wait(2000)
						if mainIni.efir['dep'] then sampSendChat('/d ['.. u8:decode(mainIni.config.c_cnn) ..'] - [СМИ] Освободил новостную волну 98.6 ФМ.') end
						wait(2000)
		...
		if smi_menu == 'ryk' then

			imgui.BeginChild('ryk', imgui.ImVec2(-1, 233 * MONET_DPI_SCALE), true)

			imgui.Text(u8' В данном разделе актуальная информация о руководстве СМИ ЛС штата Yuma.')
			imgui.Text(u8[[

]])
			imgui.Text(u8'Директор радиоцентра Nate Benson')



			if imgui.Button(u8'Написать директору') then

				if not isMonetLoader() then

					shell32.ShellExecuteA(nil, 'open', 'https://vk.com/id770453865', nil, nil, 1)

				else

					openLink("https://vk.com/id770453865")
			...
			imgui.Text(u8'Заместитель директора Bryan Benson')

			if imgui.Button(u8'Написать Брайну Бенсону') then

				if not isMonetLoader() then

					shell32.ShellExecuteA(nil, 'open', 'https://vk.com/nightooes', nil, nil, 1)

				else

					openLink("https://vk.com/nightooes")
			...
			imgui.Text(u8'Заместитель директора Chase Turner')

			if imgui.Button(u8'Написать Чейзу Турнеру') then

				if not isMonetLoader() then

					shell32.ShellExecuteA(nil, 'open', 'https://vk.com/wasd12390', nil, nil, 1)

				else

					openLink("https://vk.com/wasd12390")
...
Скрипт был разработан для Arizona RP]])
			...
			if imgui.Button(u8'Проверить обновления') then
				...
				buttonupdate('https://raw.githubusercontent.com/nightooes/script/main/version.json','[SMI-plalkeo]{FFFFFF}','url')
			...
			if imgui.Button(faicons('LOCK') .. u8' Чейнджлог', (imgui.ImVec2(200 * MONET_DPI_SCALE, 20 * MONET_DPI_SCALE))) then smi_menu = 'changelog' end
...
function main()
	...
	while true do wait(0)
		...
		if sampIsLocalPlayerSpawned() and spawn then
			...
			sampAddChatMessage("[ИНФОРМАЦИЯ] {FFFFFF}Последее обновление: "..curcolor.."{FFFACD} 17.07.2024", curcolor1)
