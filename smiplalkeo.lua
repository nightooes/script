			...
			if imgui.Button(u8'Начать эфир', (imgui.ImVec2(200 * MONET_DPI_SCALE, 20 * MONET_DPI_SCALE))) then
				...
				if mainIni.config.c_nick ~= nil then
					...
					lua_thread.create(function()
						...
						sampSendChat('/news [Собеседование] Также мы ввели новшество — дождитесь! Об этом я расскажу позже.')
						...
						sampSendChat('/news [Собеседование] Хочу сообщить вам о нововведении...')

						wait(6000)

						sampSendChat('/news [Собеседование] ..Теперь мы принимаем через собеседование сразу на 3-ю порядковую должность! ')
...
function main()
	...
	while true do wait(0)
		...
		if sampIsLocalPlayerSpawned() and spawn then
			...
			sampAddChatMessage("[ИНФОРМАЦИЯ] {FFFFFF}Последее обновление: "..curcolor.."{008000} 10.07.2024", curcolor1)
