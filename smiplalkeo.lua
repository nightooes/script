script_version('3.10 Public Version')
...
pro_vopr_text = {u8'Как обозначаются авто в объявлениях?', u8'Как обозначаются мотоциклы в объявлениях?', u8'Как обозначаются самолеты в объявлениях?', u8'Как обозначаются велосипеды в объявлениях?', 
				 u8'Как отредактируете объявления:', u8'Куплю кешу на плечо', u8'Куплю мигалку за 10кк', u8'Куплю дом в лс. Срочно', 
...
function colored_button_main(r, g, b)
	...
	if smi_menu == 'default' then
		imgui.PushStyleColor(imgui.Col.Button, imgui.ImVec4(r / 255, g / 255, b / 255, 0.86))
		if imgui.Button(faicons('gears') .. u8' Основное', (imgui.ImVec2(sizeX, sizeY))) then smi_menu = 'default' end
		imgui.PopStyleColor(1)
	else
		if imgui.Button(faicons('gears') .. u8' Основное', (imgui.ImVec2(sizeX, sizeY))) then smi_menu = 'default' end
	end
	if smi_menu == 'sobes' then
		imgui.PushStyleColor(imgui.Col.Button, imgui.ImVec4(r / 255, g / 255, b / 255, 0.86))
		if imgui.Button(faicons('id_card_clip') .. u8' Собеседование', (imgui.ImVec2(sizeX, sizeY))) then smi_menu = 'sobes' end
		imgui.PopStyleColor(1)
	else
		if imgui.Button(faicons('id_card_clip') .. u8' Собеседование', (imgui.ImVec2(sizeX, sizeY))) then smi_menu = 'sobes' end
	end
	...
	if smi_menu == 'settings' then
		imgui.PushStyleColor(imgui.Col.Button, imgui.ImVec4(r / 255, g / 255, b / 255, 0.86))
		if imgui.Button(faicons('hammer') ..u8' Настройки', (imgui.ImVec2(sizeX, sizeY))) then smi_menu = 'settings' end
		imgui.PopStyleColor(1)
	else
		if imgui.Button(faicons('hammer') ..u8' Настройки', (imgui.ImVec2(sizeX, sizeY))) then smi_menu = 'settings' end
	end
	...
	if smi_menu == 'pro' then
		imgui.PushStyleColor(imgui.Col.Button, imgui.ImVec4(r / 255, g / 255, b / 255, 0.86))
		if imgui.Button(faicons('list_check') .. u8' ПРО/ППЭ/Устав', (imgui.ImVec2(sizeX, sizeY))) then smi_menu = 'pro' end
		imgui.PopStyleColor(1)
	else
		if imgui.Button(faicons('list_check') .. u8' ПРО/ППЭ/Устав', (imgui.ImVec2(sizeX, sizeY))) then smi_menu = 'pro' end
	end
	if smi_menu == 'about' then
		imgui.PushStyleColor(imgui.Col.Button, imgui.ImVec4(r / 255, g / 255, b / 255, 0.86))
		if imgui.Button(faicons('server') .. u8' О скрипте', (imgui.ImVec2(sizeX, sizeY))) then smi_menu = 'about' end
		imgui.PopStyleColor(1)
	else
		if imgui.Button(faicons('server') .. u8' О скрипте', (imgui.ImVec2(sizeX, sizeY))) then smi_menu = 'about' end
	end
		...
		if smi_menu == 'default' then
			imgui.BeginChild('a', imgui.ImVec2(-1, 233 * MONET_DPI_SCALE), true)
				...
				imgui.Text(u8'СМИ: '..mainIni.config['c_city'] .. ' ['.. mainIni.config['c_cnn'] ..']')
							...
							imgui.Text(u8[[
VK руководителей:]])
														imgui.TextColoredRGB('VK Nate Benson: ' .. curcolor .. 'vk.com/id770453865')
			if not isMonetLoader() then
				if imgui.IsItemHovered() then imgui.SetTooltip(u8"Кликните ЛКМ, чтобы скопировать, или ПКМ, чтобы открыть в браузере")  end
				if imgui.IsItemClicked(0) then setClipboardText("https://vk.com/id770453865") end
				if imgui.IsItemClicked(1) then shell32.ShellExecuteA(nil, 'open', 'https://vk.com/id770453865', nil, nil, 1) end
			else
				if imgui.IsItemClicked(0) then openLink("https://vk.com/id770453865") end
			end
																	imgui.TextColoredRGB('VK Bryan Benson: ' .. curcolor .. 'vk.com/nightooes')
			if not isMonetLoader() then
				if imgui.IsItemHovered() then imgui.SetTooltip(u8"Кликните ЛКМ, чтобы скопировать, или ПКМ, чтобы открыть в браузере")  end
				if imgui.IsItemClicked(0) then setClipboardText("https://vk.com/nightooes") end
				if imgui.IsItemClicked(1) then shell32.ShellExecuteA(nil, 'open', 'https://vk.com/nightooes', nil, nil, 1) end
			else
				if imgui.IsItemClicked(0) then openLink("https://vk.com/nightooes") end
			end
																				imgui.TextColoredRGB('VK Chase Turner: ' .. curcolor .. 'vk.com/wasd12390')
			if not isMonetLoader() then
				if imgui.IsItemHovered() then imgui.SetTooltip(u8"Кликните ЛКМ, чтобы скопировать, или ПКМ, чтобы открыть в браузере")  end
				if imgui.IsItemClicked(0) then setClipboardText("https://vk.com/wasd12390") end
				if imgui.IsItemClicked(1) then shell32.ShellExecuteA(nil, 'open', 'https://vk.com/wasd12390', nil, nil, 1) end
			else
				if imgui.IsItemClicked(0) then openLink("https://vk.com/wasd12390") end
			end
			imgui.EndChild()
		end
			...
			if imgui.Button(u8'Начать эфир', (imgui.ImVec2(100 * MONET_DPI_SCALE, 20 * MONET_DPI_SCALE))) then
				if mainIni.config.c_nick ~= nil then
					lua_thread.create(function()
						...
						wait(6000)
						sampSendChat('/news '..u8:decode(mainIni.tags['math'])..' Даю 30 секунд на подготовку!')
						wait(6000)
					end)
				else
		...
		if smi_menu == 'country_efir' then 
			...
			imgui.SameLine()
			imgui.Ques("Как правильно проводить эфир Столицы?\n1.В новом обновлении....")
			imgui.PushItemWidth(100 * MONET_DPI_SCALE)
			imgui.InputText(u8'Денежный приз(без $)', money_math, ffi.sizeof(money_math))
			imgui.PopItemWidth()
			if imgui.Button(u8'Начать эфир', (imgui.ImVec2(100 * MONET_DPI_SCALE, 20 * MONET_DPI_SCALE))) then
				if mainIni.config.c_nick ~= nil then
					lua_thread.create(function()
						...
						wait(6000)
						sampSendChat('/news '..u8:decode(mainIni.tags['country'])..' Призовой фонд составляет целых')
						wait(6000)
						sampSendChat('/news '..u8:decode(mainIni.tags['country'])..' Даю 30 секунд на подготовку!')
						wait(6000)
					end)
				else
					sampAddChatMessage("[SMI-plalkeo] {FFFFFF}У вас не введен ник-нейм, зайдите в "..curcolor.."/smi {FFFFFF}и в настройках измените его!", curcolor1)
		...
		if smi_menu == 'sobes_efir' then
			...
			imgui.Text(u8'Будьте внимательнее! Скрипт сам проведет эфир')
			if imgui.Button(u8'Начать эфир', (imgui.ImVec2(200 * MONET_DPI_SCALE, 20 * MONET_DPI_SCALE))) then
				if mainIni.config.c_nick ~= nil then
					lua_thread.create(function()
						...
						sampSendChat('/news •°•°•°•° Музыкальная заставка радиостанции « СМИ г. Лос-Сантос » •°•°•°•°•')
						wait(2000)
						sampSendChat('/d [CNN LS] - [MM] освобождаю новостную волну 98.6 ФМ.')
						wait(2000)
		...
		if smi_menu == 'himia_efir' then 
			...
			imgui.CenterTextColoredRGB('ДАННЫЙ ЭФИР ЗАПРЕЩЕН НА СЕРВЕРЕ Yuma')
			imgui.PushItemWidth(100 * MONET_DPI_SCALE)
			imgui.InputText(u8'Денежный приз(без $)', money_math, ffi.sizeof(money_math))
			imgui.PopItemWidth()
			if imgui.Button(u8'Начать эфир', (imgui.ImVec2(100 * MONET_DPI_SCALE, 20 * MONET_DPI_SCALE))) then
...
function main()
	...
	while true do wait(0)
		...
		if sampIsLocalPlayerSpawned() and spawn then
			...
			sampAddChatMessage("[ИНФОРМАЦИЯ] {FFFFFF}Последее обновление скрипта: "..curcolor.."{88BDE1} 18.07.2024", curcolor1)
