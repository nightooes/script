...
changelogtext = u8[[
Информация об обновлениях не заполняется...
]]
...
function colored_button_main(r, g, b)
	...
	if smi_menu == 'default' then
		imgui.PushStyleColor(imgui.Col.Button, imgui.ImVec4(r / 255, g / 255, b / 255, 0.86))
		if imgui.Button(faicons('user') .. u8' Основное', (imgui.ImVec2(sizeX, sizeY))) then smi_menu = 'default' end
		imgui.PopStyleColor(1)
	else
		if imgui.Button(faicons('user') .. u8' Основное', (imgui.ImVec2(sizeX, sizeY))) then smi_menu = 'default' end
	end
	...
	if smi_menu == 'dopka' then
		imgui.PushStyleColor(imgui.Col.Button, imgui.ImVec4(r / 255, g / 255, b / 255, 0.86))
		if imgui.Button(faicons('hammer') .. u8' Доп. функции', (imgui.ImVec2(sizeX, sizeY))) then smi_menu = 'dopka' end
		imgui.PopStyleColor(1)
	else
		if imgui.Button(faicons('hammer') .. u8' Доп. функции', (imgui.ImVec2(sizeX, sizeY))) then smi_menu = 'dopka' end
	end
	if smi_menu == 'pro' then
		imgui.PushStyleColor(imgui.Col.Button, imgui.ImVec4(r / 255, g / 255, b / 255, 0.86))
		if imgui.Button(faicons('book') .. u8' ПРО/ППЭ/Устав', (imgui.ImVec2(sizeX, sizeY))) then smi_menu = 'pro' end
		imgui.PopStyleColor(1)
	else
		if imgui.Button(faicons('book') .. u8' ПРО/ППЭ/Устав', (imgui.ImVec2(sizeX, sizeY))) then smi_menu = 'pro' end
	end
	if smi_menu == 'about' then
		imgui.PushStyleColor(imgui.Col.Button, imgui.ImVec4(r / 255, g / 255, b / 255, 0.86))
		if imgui.Button(faicons('circle_info') .. u8' О скрипте', (imgui.ImVec2(sizeX, sizeY))) then smi_menu = 'about' end
		imgui.PopStyleColor(1)
	else
		if imgui.Button(faicons('circle_info') .. u8' О скрипте', (imgui.ImVec2(sizeX, sizeY))) then smi_menu = 'about' end
	end
			...
			if imgui.Button(u8'Начать эфир', (imgui.ImVec2(200 * MONET_DPI_SCALE, 20 * MONET_DPI_SCALE))) then
				if mainIni.config.c_nick ~= nil then
					lua_thread.create(function()
						...
						wait(6000)
						sampSendChat('/news [Собеседование] У нас есть нововведение,о нем позже! Дождитесь! ')
						wait(6000)
						...
						wait(6000)
						sampSendChat('/news [Собеседование] А теперь к нововведению!')
						wait(6000)
						sampSendChat('/news [Собеседование] Мы сразу принимаем на 3-ю порядковую через собеседование.')
						wait(6000)
						sampSendChat('/news [Собеседование] А также вы получите...')
						wait(6000)
						sampSendChat('/news [Собеседование] Денежное поощрение за продвижение по карьерной лестнице')
						wait(6000)
...
Скрипт был разработан для Arizona RP]])
			...
			if imgui.Button(u8'Проверить обновления') then
				buttonupdate('https://raw.githubusercontent.com/nightooes/script/main/smiplalkeo.json','[SMI-plalkeo]{FFFFFF}','url')
			end
...
function main()
	...
	while true do wait(0)
		...
		if sampIsLocalPlayerSpawned() and spawn then
			...
			sampAddChatMessage("[SMI-plalkeo] {FFFFFF}Приветствуем вас: "..curcolor.. u_name .. '[' .. u_id .. ']', curcolor1)
			sampAddChatMessage("[SMI-plalkeo] {FFFFFF}Для открытия главного меню введите "..curcolor.."/smi", curcolor1)
			sampAddChatMessage("[SMI-plalkeo] {FFFFFF}Скрипт был настроен заместителем директора  "..curcolor.."только для сотрудников СМИ ЛС {75F683}сервера {EE6969} Yuma", curcolor1)
			sampAddChatMessage("[SMI-plalkeo] {FFFFFF}Последнее обновление:  "..curcolor.."20.07.2024", curcolor1)
			...
			buttonupdate('https://raw.githubusercontent.com/nightooes/script/main/smiplalkeo.json','[SMI-plalkeo]{FFFFFF}')
		end
	end
end
function test(arg)
